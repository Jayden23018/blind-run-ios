...
enum LanternMap {
    ...
    ///
    /// 🔑 **换四叉树那天，网格尺寸有一个可算的下限，别凭感觉调。**
    /// 2026-09-21 用 `scripts/render-lantern-scale-preview.sh` 量出来的：
    /// 全国视图下 390pt 宽承载 63 个经度 ⇒ 6.19 pt/度；一盏灯的光晕直径约 `8.4 × r`，
    /// 最小 r 是 2.7（一个人）⇒ 22pt ⇒ **相邻光点不重叠要求网格 ≥ 3.5°**。
    ///
    /// 直接后果：**区县粒度（0.25°）在全国视图下必然糊成一团**，这不是画法问题，
    /// 是粒度问题 —— 同一轮实测里，5000 人按 0.25° 聚出 1528 处，长三角、京津冀、
    /// 珠三角连成三坨纯白，底层灯火完全消失。所以 `site` 档的阈值
    /// （`siteTierScale = 2.5`）在**真实数据**下是偏低的：`scale` 要到大约 14×
    /// 才撑得开区县粒度。假数据只有 26 个点，碰不到这条线。
