...
struct VolunteerTabView: View {
    ...
    var body: some View {
        ...
        // `TabView` 在 UIKit 侧是一个 `UITabBarController`：SwiftUI 的 hidden 只标在包它的那个
        // 节点上，底下那棵 UIKit 树（首页 ScrollView、标签栏）照样暴露 —— 2026-09-23 真机
        // UI hierarchy 实测，`volunteerProfileIdentityRow` 与 `TabBar` 在卡片弹出时都还在。
        // 与 `AMapContainer.makeUIView` 直接设 `accessibilityElementsHidden` 是同一个原因。
        // 这行留着管的是横幅与撤销 toast 那两层纯 SwiftUI 的 overlay。
        // 用例：`testMockVolunteerInviteSheetExposesItsThreeActions`（断言首页那一行不在树里）。
...
/// 再往下找它的 `UITabBarController` 子控制器。标志挂在 tab bar controller 的根视图上，
/// 所以三个 tab、每个 tab 里 push 出去的二级页和标签栏本身一次全盖住。
private struct TabBarAccessibilityHider: UIViewRepresentable {
    ...
    func updateUIView(_ uiView: UIView, context: Context) {
        ...
        DispatchQueue.main.async {
            Self.tabBarController(near: uiView)?.view.accessibilityElementsHidden = isHidden
        }
    }
