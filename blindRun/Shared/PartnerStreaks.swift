...
enum PartnerStreakCopy {
    ...
    /// ⚠️ 空态只说火花怎么来的：那条路不需要任何前置操作，一起跑完订单就会结算。
    ///
    /// > 原注释的理由是「本仓库没有收藏入口，写成『就可以把他设为固定搭档』是承诺一个不存在的功能」。
    /// > 入口现在有了（已完成订单的详情页），但这句文案**仍然不提它** —— 它是**火花列表**的空态，
    /// > 而火花与收藏是两条独立的路。指向另一条路会让用户以为收藏能让这一屏长出东西来。
