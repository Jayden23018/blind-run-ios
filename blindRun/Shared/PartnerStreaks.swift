...
///
/// 🚩 **为什么必须合并两个端点，而不是只读固定搭档列表**：
/// 火花是由「一起跑完的订单」结算出来的，**与收藏无关、不需要任何前置操作**；
/// 而收藏要用户主动点一次。只读收藏列表的话，从没点过收藏的人这一屏永远是空的，
/// 哪怕他和某位志愿者已经连跑了七周。
///
/// > 2026-08-23 这里写的是「本仓库根本没有收藏入口，所以只读收藏列表会永远空」。
/// > 那句已作废：收藏入口现在有两个（本页的按钮、已完成订单详情页），
/// > 但合并的理由换成上面那条之后依然成立。
///
...
enum PartnerStreakCopy {
    ...
    /// 空态分三档，判据是 `GET /api/config/features` 的 `partnerStreakEnabled`。
    ///
    /// 🔴 **同一个空数组有两个完全相反的含义**，而只有后端知道是哪一个：
    ///
    /// | 真实情况 | 该说的话 |
    /// |---|---|
    /// | 开关关着 | 「功能还没开放」—— 他不该觉得是自己的问题 |
    /// | 开关开着、他确实还没点亮 | 「连续两周一起跑步，这里会显示连续记录」 |
    /// | 拿不到开关 | 只陈述事实，两种情况下都成立的那句 |
    ///
    /// 此前恒说第二句。开关关着的那段时间里，那句话是在**教用户去做一件做了也不会有结果的事** ——
    /// 对读屏用户尤其糟：他会照着做两周，然后回来发现还是空的。
    /// （后端 `app.incentive.streak.enabled` 默认 `false`，也就是说那句话从上线起就一直是错的。）
    ///
    /// 🚩 `nil` 档**不能退回第二句**。拿不到开关时我们不知道是哪种情况，
    /// 而第二句是一个承诺；说一句只陈述现状的话，两种情况下都不算骗人。
    ///
    /// ⚠️ 三档都**不提收藏入口**（已完成订单详情页那个）—— 这是**火花列表**的空态，
    /// 而火花与收藏是两条独立的路：火花由订单结算，收藏要用户主动点。
    /// 指向另一条路会让用户以为收藏能让这一屏长出东西来。
    static func blindEmpty(streakEnabled: Bool?) -> String {
        switch streakEnabled {
        case true:
            return "还没有可以显示的搭档。和同一位志愿者连续两周一起跑步之后，这里会显示你们的连续记录。"
        case false:
            return "还没有可以显示的搭档。连续一起跑步的记录功能还没有开放，开放之后这里会显示你和同一位志愿者连续跑步的周数。"
        case nil:
            return "还没有可以显示的搭档。"
        }
    }

    static func volunteerEmpty(streakEnabled: Bool?) -> String {
        switch streakEnabled {
        case true:
            return "还没有跑者把你设为固定搭档，也还没有连续一起跑步的记录。"
        case false:
            return "还没有跑者把你设为固定搭档。连续一起跑步的记录功能还没有开放。"
        case nil:
            return "还没有跑者把你设为固定搭档。"
        }
    }
