...
//     而那是本仓库最常见的干法。
//     ⚠️ 别改用「HEAD 作者 == `git config user.name`」当豁免：本仓库全部提交的 author
//     都是同一个人，**含事故里同事那条 `dd0d795`** —— 那条判据恒成立，等于把这一整条判据废掉，
//     而且不会有任何东西提示它已经废了。
//
// ── ③ unguarded-checkout-chain 的判据（2026-09-06 立）──
...
//
//   起因：2026-09-06 一天里中了两次，`git merge origin/main` 分别落到了另外两个 PR 的分支上
//   （两次都碰巧无害，那是运气）。根因是本仓库常年挂着十几二十个 worktree ——
//   主工作区 checkout 失败是**常态**，不是意外。
//
