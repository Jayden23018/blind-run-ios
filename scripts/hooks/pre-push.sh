...
# 本仓库的 pre-push 正文。由 scripts/install-git-hooks.sh 装的壳 exec 进来，跟着当前分支走。
# 试跑：bash scripts/hooks/pre-push.sh < /dev/null
#
# 跳过：AIDRUN_SKIP_PREPUSH=1 git push（或 git push --no-verify，两仓通用）
...
fail=0
# 有没有门禁被跳过。跳过不等于失败（离线、没有后端 checkout、本分支还没有某个校验脚本都是合理的），
# 但**绝不能汇报成「全部通过」**。run_node 的跳过以前不计数，于是照样打印「全部通过」（A3）。
SKIPPED_GATES=0
# 日志每次一个目录：两个 worktree 同时 push 时，固定的 /tmp 日志文件会互相覆盖（B4）。
PREPUSH_TMP="$(mktemp -d 2>/dev/null)" || PREPUSH_TMP="/tmp/aidrun-prepush.$$"
mkdir -p "$PREPUSH_TMP"
LOG="$PREPUSH_TMP/gate.log"
run() {
  echo "[pre-push] $1"
  shift
...
BACKEND_TMP="$PREPUSH_TMP/contract"
mkdir -p "$BACKEND_TMP"
...
if [ -f "$SPEC" ]; then
  ...
  if scripts/generate-api-client.sh "$SPEC" >"$LOG" 2>&1; then
    ...
    echo "[pre-push] ✗ 生成失败："; tail -n 15 "$LOG"; fail=1
  fi
else
...
if [ "$fail" -ne 0 ]; then
  echo "[pre-push] 校验未通过，push 已中止。确需绕过：AIDRUN_SKIP_PREPUSH=1 git push（或 git push --no-verify），绕了要说明"
  exit 1
fi
