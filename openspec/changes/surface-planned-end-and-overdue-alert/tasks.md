...
- [x] 3.1 `blindRunTests/PlannedEndAndOverdueTests.swift`（12 条）：结束时间取 `plannedEnd`
...
- [ ] 3.3 真机跑（范围按符号搜定：`PlannedEndAndOverdueTests` /
      `NotificationCatchUpTests` / `KeepWaitingTests` / `blindRunTests`）。
- [x] 3.4 编译门禁 `build-for-testing` —— **TEST BUILD SUCCEEDED**。
- [ ] 3.5 `validate-guard` / `validate-docs` / `validate-spec-coverage` /
      `validate-error-codes` / `openspec validate --strict`。

## 4. 契约投递（后端仓库 `demo/docs/handoff.md`）

- [ ] 4.1 答问题①：`ORDER_OVERDUE` 收到会怎么表现（通用通知通道 → 前台横幅 + TTS，
      未被生命周期闸抑制；本变更抬为高优先级）。
- [ ] 4.2 答问题②：**没有**按 `plannedEndTime` 本地推断订单已结束的代码，全仓核过。
- [ ] 4.3 🔴 提问：盲人侧 `ORDER_OVERDUE` 模板是 `NORMAL`（`websocket-protocol.md:158`），
      而 APNs 只补发 `HIGH`（`NotificationService.java:134`）—— App 不在前台时盲人**收不到**
      超时告警，而志愿者侧是 `HIGH` 收得到。「跑者可能失联」唯一收不到的是跑者本人。
      请把盲人侧模板提到 `HIGH`。
- [ ] 4.4 提问：志愿者侧文案还写「订单已超过结束时间**1小时**」（`websocket-protocol.md:451`），
      N63 之后实际是 15 分钟。
- [ ] 4.5 提问：盲人侧文案「志愿者会尽快结束服务」假定志愿者在场且会处理，
      而这条告警的语义是「跑者可能失联」。
...
- [ ] 5.1 真机开 VoiceOver：`IN_PROGRESS` 时按「重复当前状态」听得到结束时间；
      状态卡的合并朗读里也有。
- [ ] 5.2 低视力档位（AX3 以上字号）看一眼状态卡多这一行不裁切。
