...
- Present backend `ESCORT_DISTANCE_ALERT` as a high-priority visible, VoiceOver, and TTS warning without changing order status.
...
- Backend dependency: GCJ-02 request/response semantics are confirmed in direction but still being modified; exact conversion boundary, existing stored-track handling, heartbeat support on `/ws/volunteer`, notification envelope, and anomaly threshold/result must be frozen before cloud enablement.
