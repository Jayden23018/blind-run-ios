...
- [x] 1.1 投 handoff「待后端确认」：要不要做真的积分体系？要的话给出产生规则、
      有没有兑换出口、会不会过期。注明前端已撤掉假实现，**不阻塞**后端将来做真的。
      **已投（2026-08-15）**：后端仓库 `docs/handoff.md` 待后端确认分区末尾，
      与 1.2 合成一条，附带 `placeholder-promise` 守卫的说明。
- [x] 1.2 投 handoff：完成单数多的志愿者要不要在派单里加权（滴滴「点亮勋章后接单概率增加」）？
      这决定成就体系是纯荣誉还是有实际收益，会影响前端文案措辞。
      **已投（2026-08-15）**：同上那一条的第 ② 问。同时把「不得说成证明/证书/已认证」
      （民政部令第 67 号，守卫 `volunteer-hours-credential`）的口径一并同步了 ——
      积分体系将来若要对外作数，那条对后端同样成立。
