...
- [x] 7.7 **生产水位线已核实，这条不再是前置阻塞** —— 2026-09-21 直连生产 ECS 查得：
      `/opt/blindrun/demo-0.0.1-SNAPSHOT.jar` 构建于 **2026-09-10 10:48**，服务
      `ActiveEnterTimestamp=Mon 2026-09-14 21:20:47 CST`（`Started DemoApplication in 72.23 seconds`）。
      后端 `origin/main` 在该构建点之前的最后一个提交是 `963ba37`（2026-09-10 10:41:57），
      而 08-09 那批远早于它 ⇒ **生产跑的代码已包含 08-09 那批**，原文记的「最后一次核实是 08-06」已过期 35 天。
      ⚠️ 这条只解掉「不知道真机在验哪一版后端」这个未知，**7.6 那一下人耳验证仍未做**，别把这个勾当成跨轮修正验过了。
