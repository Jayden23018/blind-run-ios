...
- [ ] 2.4 **第二条接单前的路**：串行派单之外还有抢单式兜底 —— 「可接订单」列表
      （`VolunteerOrderFlowViews.swift:440`）调 `GET /api/orders/available`。
      卡片 `VolunteerAvailableOrderCard` 目前只渲染 `routeNotes`（`:2165`），**屏幕上不漏**，
      但该端点在契约里是裸 `type: object`（`api_spec.yaml:1899-1905`），
      返不返 `specialNotes` 无从判断 —— 数据可能已经到设备上了。已随 ⑥ 投 handoff。
- [ ] 2.5 `routeNotes`（路线备注）同为自由文本、同在接单前可见（`:2165` / `:2557`），
      按 `design.md` D2 的判据本该也归到接单后。**本轮不改**：它只有表单入口、语音不写它，
      量级不同。但要在 §4 文档里写明这是**已知的、有意留下的**边界，别让它变成绕行通道。
