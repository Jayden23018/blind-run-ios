...
AidRun 当前仓库是原生 iOS 客户端，采用 SwiftUI + MVVM，后端为仓库外云端服务 `https://47.114.113.171`，WebSocket 使用 `wss://47.114.113.171`，地图能力来自高德 iOS SDK。Mock 仍保留为离线 UI 和单元测试设施，但不能作为上线验收依据。
