...
| 盲人紧急联系人管理 | P0 | 1～5 个联系人的列表、新增、编辑、删除、设为主联系人；必须恰好 1 个主联系人；最后 1 个联系人不可删除。**「至少 1 个联系人且恰好 1 个 primary」是下单硬前置条件**，与后端 `OrderCreationService` 的校验一致（缺失时 403 `EMERGENCY_CONTACT_REQUIRED`） |
| 盲人实名认证 | P1 | 提交身份证姓名和号码做二要素核验，展示 `NOT_VERIFIED` / `VERIFIED` / `FAILED` 三态（无「审核中」态）。**2026-07-30 起是下单硬门槛**：后端 `OrderCreationService` 要求 `verifyStatus == VERIFIED`，否则 403 `IDENTITY_NOT_VERIFIED`，且这一档排在紧急联系人校验**之前**（`demo/docs/handoff.md` Q1 按方案 ① 答复） |
...
| 禁定位降级 | P0 | 拒绝定位权限时，盲人**仍可预约**：出发地点改由手动搜索（高德 POI 不需要定位权限），同时给出可听可见的降级告知（志愿者看不到实时位置、云端求助可能发不出去）。志愿者侧不变：不能接收或接受依赖位置的系统派单 |
...
| 危险操作二次确认 | P0 | 取消订单、退出登录、一键求助。求助确认文案固定为「是否确认进入求助状态？确认后，本次服务将标记为异常，系统会记录当前订单状态。」，取消则什么都不发送。**结束陪跑自 2026-09-16 起不走弹窗，改为长按 2 秒**（见下一行）|
| 结束陪跑长按 2 秒 | P0 | 志愿者端唯一的结束入口：长按满 2 秒才结束，环形进度 ⌀40 走满、按住过程渐强震动、**松手即取消**（不播报、无提示）。**没有轻点路径** —— 结束即不可撤销，长按本身就是那道确认，再叠一个弹窗只会让人习惯性点掉。读屏用户走自定义无障碍动作「结束陪跑」。落点 `VolunteerFinishLongPress`（2026-09-16，设计包 `状态清单.md` §11）|
...
| 15 | 志愿者长按"结束陪跑"2 秒 | 手机 B | 订单状态变为 COMPLETED。中途松手不结束，也不提示 |
...
3. GPS 使用设备真实当前位置；当前订单双方在批准状态内可查看对方不超过 15 秒的新鲜位置。
...
- 两角色 30 秒心跳、服务阶段 5 秒真实位置、15 秒过期策略和 500 ms 串行发送。
- `IN_PROGRESS` 开启系统后台位置能力并显示披露/降级状态，离开运行状态立即关闭。
- 双向同行 marker、`ESCORT_DISTANCE_ALERT` / `ESCORT_SIGNAL_LOST` 高优先级提示，以及完成后“本次路线”。
- 后端无历史坐标标记/迁移，且轨迹异常阈值未版本化，均列为发布风险而非客户端猜测。
