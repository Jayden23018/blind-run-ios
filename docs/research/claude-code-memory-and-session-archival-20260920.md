...
### 6.1 本次调研自己的成本基线（以后定档拿这个对照）

`get_usage` 实测，一次 **C 档三线并行**调研的真实账单：

| 项 | tokens |
|---|---|
| subagent · 官方记忆机制 | 192,356（20 次工具调用） |
| subagent · claude-mem 与竞品 | 147,848（34 次） |
| subagent · NotebookLM | 143,524（24 次） |
| **subagent 小计** | **483,728（占总量 60%）** |
| 主会话上下文 | 315,277（Messages 253,698） |
| **合计** | **≈ 800k** |

⇒ 吃掉 **Max 5x 五小时额度的 13%**，周额度（全模型）同期到 58%。

**两条可复用的判据**：

1. 🔴 **subagent 不省额度，只省窗口。** 它读不到父会话缓存（官方：subagent 落在 5 分钟 TTL 桶外，
   且不继承前缀），每个都是独立全价；报告回传后又整份进主上下文。要省额度只有「不派」。
2. **先盘点再定档能省约 37%。** 本轮的真实失误：先开了三线，**回头才查本机**——
   而本机五条命令就把 claude-mem 那条线的答案全给了（§1），那条线的 147,848 tok 本可不花。

已落回全局 `~/.claude/CLAUDE.md` 的「委派」节，并补了一条「≥3 个 subagent 或预计 >200k
先报预算」。
