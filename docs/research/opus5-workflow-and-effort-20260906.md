...
> ✅ **2026-09-21 复核：本节全部成立，来源 URL 返回 200。** 逐条核对过官方文档现行版本 ——
> 「评估器不跑命令也不读文件」那句逐字一致、4,000 字符上限、`or stop after 20 turns`、
> met/impossible 裁决、后台任务跳过该轮评估，全部对得上；官方另给了 `/goal` vs `/loop` vs
> Stop hook 的三方对照表与 `◎ /goal active` 指示器。
>
> 🔴 **但本节曾被误判为「整节作废」，而根因与本报告被关在 PR #108 里直接相关**：
> 2026-09-19 的 [`claude-agent-view-for-cross-repo-20260919.md`](./claude-agent-view-for-cross-repo-20260919.md)
> §3 断言「`/goal` 不存在、官方 `code.claude.com` 查不到」—— 而它要找的那个确切 URL
> **就写在下面这一行**。它没看见，因为本报告在 PR #108 的分支上、不在 `origin/main`、
> 也不在 `docs/research/INDEX.md` 里，于是那轮调研照 `AGENTS.md` §12 读了索引、
> 没找到、重做一遍，**得出了相反的错误结论并被引用放大**。
> 详见那份报告 §3 的订正块与 [`../review/orphan-branch-triage-20260921.md`](../review/orphan-branch-triage-20260921.md)。

来源：[Keep Claude working toward a goal](https://code.claude.com/docs/en/goal)（2026-09-21 复核 HTTP 200）
...
| 6 | `AGENTS.md` §9–§13 迁进 skill，§1 索引压成路由表 | 本仓库 | ⚠️ **未落地**（原写「✅ 本轮完成（PR #109）」，而 **#109 已于 2026-09-05 关闭、未合并**；`origin/main` 的 `AGENTS.md` 现为 501 行、`aidrun-hooks-and-guards` skill 不存在。见下方订正） |
| 7 | `MEMORY.md` 条目钩子压回一行 | 记忆目录 | ✅ **本轮完成**（最长 17 条，14,749→11,443 字符） |
| 8 | 真机测试用 `/goal` | 用法 | ⚠️ **未落地到主线**（写法确实写好了，但落在 PR #109 的分支上 —— `origin/main` 的 `aidrun-ship-check/SKILL.md` 里 `/goal` 零命中。⚠️ 那节内容本身是**对的**，见 §C.3 的 09-21 复核） |

> ⚠️ **2026-09-21 补：上面 #6 / #8 两处原本都写着「✅ 本轮完成」，而主线上都没有。**
> 它们兑现在 PR #108 / #109 的分支上，两个 PR 同日（2026-09-05）被关闭、各 0 条评论。
> ⇒ **调研报告里的「✅ 本轮完成」指的是「我在某个分支上做了」，不等于主线上有。**
> 引用任何一条「已落地」之前，在 `origin/main` 上当场核一眼
> （同型见记忆 `pr-target-branch-is-main-not-integrate` 的 08-15 节：文档里的「PR #N」不是证据）。
