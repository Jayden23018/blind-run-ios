...
| 2026-08-13 | 怎么在腾讯会议上同时演示两台 iOS 真机（盲人端 + 志愿者端）？ | 腾讯会议**没有**「手机投屏当共享源」，只能桌面/窗口/白板；正解是 QuickTime「新建影片录制」开**两个窗口**分别镜像两台 USB 设备，再用腾讯会议 3.30+ 的 Shift 多选窗口共享（本机 3.41.1 满足）；AirPlay 到 Mac 只能救一台、不支持双设备并存，仅作退路；⛔ **iPhone 镜像（iPhone Mirroring）出局** —— 镜像期间 iPhone 必须锁定、麦克风不可用，语音下单跑不起来；QuickTime 的 Microphone 要选「无」不要选 iPhone（会劫持音频路由）；⚠️ QuickTime 镜像有中途断线且**只能重启 Mac** 恢复的已知 bug，直播演示必须备录屏 | 腾讯会议新增移动设备作为共享源；macOS 大版本改 QuickTime 设备捕获行为或修掉镜像中断 bug；演示设备组合变化 | [tencent-meeting-two-ios-device-demo-20260813.md](./tencent-meeting-two-ios-device-demo-20260813.md) |
