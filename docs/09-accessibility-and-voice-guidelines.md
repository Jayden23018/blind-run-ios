...
- 语音下单的读回整单：**用户原话 + 最终要下的单（出发地点 + 结束地点 + 预约时间 + 时长）+ "说「确认」就下单；要改就说「改地点」「改时间」或者「改时长」"两条出路**。三段缺一不可 —— 没有原话分不清是自己说错还是系统听错；没有整单不知道默认值补了什么；没有出路，看不见屏幕的人不会自己发现还能改
...
- 语音下单的时长取整：用户说的分钟数不在 15/30/45/60/90/120 档位上时，**必须说出"你说的是 X 分钟，本应用按 Y 预约"**，且这句要和读回拼成同一段（分两次播会让「重复一遍」念不到取整提示）。静默取整对听不见屏幕的人就是篡改
