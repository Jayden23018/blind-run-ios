...
6. 必须在真机 `111` 上通过真实高德 smoke、真实云端 UI smoke 和后端 E2E。
7. 固定验证码、HTTP 明文 IP、UserDefaults token、志愿者认证简化流程等风险必须在 `plan.md` 中记录并获得人工确认。
...
| 短信验证 | 当前使用固定验证码 `000000`；真实短信或等价验证为上线风险项 |
| 志愿者认证 | 当前为简化认证；真实实名认证/审核按后端契约接入 |
