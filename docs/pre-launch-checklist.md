...
- [ ] 🔴 **确认有付费 Apple Developer Program 账号**（$99/年）并能访问 App Store Connect。⚠️ 工程文件里写死的 `DEVELOPMENT_TEAM = R6PH2TFB3Q` 是原开发者的团队号；本机用 `ZW39BS8NXT`。**需要确认哪个团队持有 App Store Connect 记录**（见 §H）。
...
- [ ] 🟡 决定内部（≤100 人、免审）还是外部（≤10,000 人、需 Beta App Review）。首轮若只发给团队与 2–3 位盲人志愿者，内部通道能省掉一次审核等待。
...
- [ ] 🟡 每位测试者配一名陪同人员（首次使用陌生 App + 户外陪跑，双重风险）。
...
- [ ] **导师说的「内测」是 TestFlight 分发，还是先在线下用我们自己的设备陪同试用？** 后者能跳过整个 §C1 和 §D 的第一条，是更快的第一步。
