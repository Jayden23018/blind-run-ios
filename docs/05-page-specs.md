...
**主要内容**：
- 用户信息区（昵称、接单状态、附近需求数、定位摘要）
- 可服务开关（Toggle，新志愿者默认关闭，由用户手动开启；seed demo 账号可预置开启）
- 高德地图全屏背景（显示当前位置和附近订单位置点；隐藏指南针）
...
- 点击订单卡片 → 订单详情页
- 点击"查看全部" → 订单列表页
- 拖动附近需求面板 → 在收起 / 默认 / 展开三档间吸附
- 点击切换角色 → 角色切换（检查活跃订单）

**状态变化**：
- 可服务开关切换：即时更新（API 调用）
- 当前订单：进入首页和返回首页时通过 `/api/orders/mine` 刷新；终态订单不显示
- 订单列表：进入时加载，点击刷新按钮重新加载
- 地图当前位置：蓝点保持在顶部用户信息区和附近需求面板之间的可视区域中点
...
- 附近需求拖拽把手：accessibilityLabel = "拖动附近需求面板"，accessibilityHint = "上滑展开，下滑收起"
