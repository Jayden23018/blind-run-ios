...
**When** 页面加载完成
**Then** 首个主要内容是问候（标题），其后是浅蓝「预约新的陪跑」块（最小高度 64pt）；"重复当前状态"在问候行右侧（~~"开始约跑"按钮~~ 2026-09-16 起改为预约块；~~角色切换入口~~ **【未决 / 未实现】入口已删除**，见 US-ROLE-002）
**And** ~~当前位置文字和高德地图作为辅助位置确认展示在主操作之后~~ **首页自 2026-09-16 起不含地图与位置摘要行**（位置信息在订单卡的地点行里，位置摘要仍进播报）；读屏顺序是 问候 → 重复当前状态 → 订单卡 → 预约块 → 标签栏
...
**When** 页面加载完成
**Then** 首个主要内容改为深蓝订单卡（**整张卡一个按钮、一个无障碍元素**，含状态、时间、地点、陪跑员），其下仍保留预约块 —— 后端允许同时握着多张未走完的单（上限 3，`TOO_MANY_SCHEDULED_ORDERS`），这个入口不该消失（~~不显示"开始约跑"按钮~~ / ~~"查看当前订单"~~ 整张卡就是那个动作）
**And** ~~地图或位置辅助内容不得先于订单状态…~~ 首页已无地图；且订单卡在**已经出发之后**（`DRIVER_EN_ROUTE` / `DRIVER_ARRIVED` / `IN_PROGRESS`）不许把这一单说成「下一次陪跑」、也不许在 52pt 大字位置显示已经过去的计划开始时刻
...
**Given** 订单状态为 IN_PROGRESS
**When** 志愿者长按"结束陪跑"满 2 秒（环形进度走满，按住过程渐强震动）
**Then** 订单状态变为 COMPLETED
**And** 中途松手即取消：环形归零，不播报、不提示，订单不变
**And** 读屏用户改用自定义无障碍动作「结束陪跑」，不必真按住

> 2026-09-16 改：此前是「点击 → 确认弹窗 → 可选填写服务总结」。**不做轻点**是因为结束即不可撤销，
> 长按本身就是那道确认；服务总结一并撤下 —— `POST /api/orders/{id}/finish` 没有请求体，
> 那段文字从来没离开过手机。要恢复得先让后端加字段。
...
**Given** 用户触发危险操作（取消订单、退出登录、一键求助；**结束陪跑除外 —— 它走长按 2 秒，见上文**）
**When** 点击操作按钮
