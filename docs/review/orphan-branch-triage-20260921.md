...
本轮因此**停在判定，不合并这两条**，待项目负责人回答当时为什么关。
附带查出两条独立成立的缺陷：**紧急联系人表单是全仓唯一没有焦点管理的多字段表单**（iPad 焦点问题未修），
以及 **`/goal` 死引用仍在全局 `CLAUDE.md:16`**，而 PR #109 的 skill 里还有一整节它的用法教程。
...
| `docs/opus5-workflow-research` | **#108 CLOSED** | ⏸ **停在判定** —— 待问关闭原因 | 463 行报告，结论已被全局 `CLAUDE.md` 吸收（但引的是后端姊妹篇），且 §C.3 整节建立在不存在的 `/goal` 上，见 §4 |
| `chore/slim-resident-context` | **#109 CLOSED** | ⏸ **停在判定** —— 待问关闭原因；即使重做也不能照抄 | AGENTS.md 已分叉 13 次，且其 skill 带 3 处 `/goal` 死引用，见 §5 |
...
## 4. `docs/opus5-workflow-research`（PR #108 CLOSED）：停在判定
...
**为什么不合**：PR 被主动关闭且无理由记录。此外有三条事实需要一并考虑：
...
2. 🔴 **§C.3 整节（标题写着「本仓库最有增量的一条」）建立在一个不存在的命令上。**
   `/goal` 已由 [`claude-agent-view-for-cross-repo-20260919.md`](../research/claude-agent-view-for-cross-repo-20260919.md) §3 证伪
   （`claude --help`、三个应用目录 `find`、session skill 列表三处零命中，官方 `code.claude.com` 查不到）。
   报告给出了命令写法、4000 字符上限、1h→2h 退避曲线 —— **错得足够具体，具体到会被照抄**，
   而它确实被照抄了一次（见 §5）。合并前这一节必须整节标注作废。
...
2. 🔴 **它的 `aidrun-ship-check/SKILL.md` 带 3 处 `/goal` 死引用**（`:159` 一整节标题
   「用 `/goal` 把「跑到绿」交给评估器」、`:161`、`:165` 一条具体命令写法）。
   ⇒ 合并它 = 把一个不存在的命令的完整用法教进主线。**这是 §4 第 2 条那个「错得具体所以被照抄」的落点。**
...
**② `/goal` 死引用仍在全局 `~/.claude/CLAUDE.md:16`**，09-19 发现至今未改：

```
- 终态可验证的长任务用 `/goal <条件>` —— 每轮由独立 checker 验，满足才收工，不是给指令列表
```

`origin/main` 全仓 4 处 `/goal` **全部是 09-19 那份证伪报告自己的内容**（INDEX 一行 + 报告三行），
说明主线是干净的 —— **唯一仍在误导的那一处在全局配置里**，而它每个会话都注入。
09-19 报告已记下代价：「会被任务书照抄」。
⚠️ 那是项目负责人的个人配置文件，本轮不代改，只报告。建议的替代写法：
`/loop` + 在 prompt 里写死判据（09-19 报告给出的现成办法）。
