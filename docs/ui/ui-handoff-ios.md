...
| 原则 | 细则 |
|------|------|
| 视觉 | **跟随系统明暗**（不强制深色）、高对比度文字、大字号（≥20pt body）、无多余装饰；状态和主操作先于地图。配色一律走 `AppColors`，方向见 [`design-direction.md`](./design-direction.md) |
...
| 原则 | 细则 |
|------|------|
| 视觉 | **跟随系统明暗**（与盲人端同一套 `AppColors`，不分叉），信息密度可高于盲人端，但关键按钮仍需清晰。安全相关界面退回最克制的一档，见 [`design-direction.md`](./design-direction.md) §6 |
| 地图 | 全屏或半屏高德地图，显示当前位置和订单 marker |
| 隐私 | 接单前**隐藏**盲人联系电话、紧急联系人；接单后展示**掩码号码**并给拨号入口。全号只进 `tel:`，不上屏、不进 `accessibilityLabel`（VoiceOver 是外放的，念全号等于把号码广播给周围所有人）。渲染走 `EmergencyContactResponse.maskPhone`，拨号走 `EmergencyDialer.telURL/dial`。<br>2026-09-08 更正：此处原写「接单后显示完整电话」，与 `AGENTS.md` §8 自 2026-08-22 起的口径（commit `f404de2` / 审计 F10）冲突。那次同批改了 `technical-design-overview.md` 与 `user-manual.md`，**漏了本文件** |
