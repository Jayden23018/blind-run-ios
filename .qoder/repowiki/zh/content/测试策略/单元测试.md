...
**更新** 新增语音服务 API 兼容性测试、语音输入字段允许列表测试、错误处理场景测试，以及紧急确认文案测试用例，确保紧急求助功能的文案一致性与无障碍体验。
...
- [SpeechInputService.swift:53-54](file://blindRun/Voice/SpeechInputService.swift#L53-L54)
- [SpeechService.swift:9-11](file://blindRun/Voice/SpeechService.swift#L9-L11)
...
### 语音服务测试
- 目标
  - 验证语音服务 API 兼容性、状态播报与错误处理。
- 方法
  - 测试 VoiceService 的 API 兼容性别名，确保与旧版 SpeechService 的无缝迁移。
  - 验证状态播报的正确性和去重机制，符合无障碍指南。
  - 测试语音输入字段的允许列表，确保只允许文本字段进行语音输入。
- 建议
  - 语音服务应提供单一入口，避免重复播报。
  - 语音输入应严格限制在允许的文本字段范围内。
  - 错误处理应提供清晰的键盘回退提示。

**更新** 新增语音服务 API 兼容性测试、语音输入字段允许列表测试、错误处理场景测试，确保语音功能的稳定性和无障碍体验。

**章节来源**
- [SpeechService.swift:24-73](file://blindRun/Voice/SpeechService.swift#L24-L73)
- [SpeechService.swift:77-94](file://blindRun/Voice/SpeechService.swift#L77-L94)
- [SpeechInputService.swift:86-128](file://blindRun/Voice/SpeechInputService.swift#L86-L128)
- [SpeechInputService.swift:18-21](file://blindRun/Voice/SpeechInputService.swift#L18-L21)
- [VoiceTextField.swift:42-58](file://blindRun/Voice/VoiceTextField.swift#L42-L58)

### 语音输入服务测试
- 目标
  - 验证语音输入服务的字段允许列表、错误处理和状态管理。
- 方法
  - 测试允许列表方法，确保只有文本字段被允许进行语音输入。
  - 验证非允许字段的拒绝行为，确保提供键盘回退。
  - 测试各种停止原因的状态变化和错误消息。
- 建议
  - 语音输入服务应提供清晰的错误消息和回退机制。
  - 停止原因应有明确的语义和用户友好的提示。
  - 服务应支持手动停止、超时停止和最大时长限制。

**更新** 新增语音输入字段允许列表测试、错误处理场景测试，确保语音输入功能的安全性和可靠性。

**章节来源**
- [SpeechInputService.swift:18-21](file://blindRun/Voice/SpeechInputService.swift#L18-L21)
- [SpeechInputService.swift:32-46](file://blindRun/Voice/SpeechInputService.swift#L32-L46)
- [SpeechInputService.swift:134-173](file://blindRun/Voice/SpeechInputService.swift#L134-L173)
- [VoiceTextField.swift:72-77](file://blindRun/Voice/VoiceTextField.swift#L72-L77)

### 紧急求助功能测试
- 目标
  - 验证紧急求助功能的文案一致性、确认对话框行为与无障碍体验。
- 方法
  - 测试 EmergencySafetyCopy 文案常量的准确性与完整性。
  - 验证紧急确认弹窗的标题、确认按钮、取消按钮与消息内容。
  - 确保紧急求助按钮的无障碍标签与提示正确。
- 建议
  - 紧急求助文案应简洁明确，避免歧义。
  - 确认对话框应提供明确的撤销机制。
...
**更新** 新增紧急确认文案测试用例，确保紧急求助功能的文案一致性与无障碍体验。
...
- [BlindOrderStatusView.swift:220-238](file://blindRun/BlindRunner/BlindOrderStatusView.swift#L220-L238)
...
- 测试与语音服务
  - 语音服务测试独立于主应用逻辑，通过直接实例化服务进行测试。
  - 语音输入服务测试验证字段允许列表和错误处理机制。
...
VoiceTests["语音服务测试"] --> Voice["Voice Services"]
Voice --> SpeechService["SpeechService"]
Voice --> SpeechInputService["SpeechInputService"]
```
...
- 常见问题
  ...
  - 语音服务测试失败：检查权限授权和音频会话配置。
  - 语音输入字段测试失败：验证字段 ID 的正确性和允许列表逻辑。
- 建议
  - 对错误路径进行充分测试，包括网络错误、鉴权失败、业务错误。
  - 对 TTS 与无障碍功能进行覆盖，确保用户体验一致性。
  - 语音服务测试应模拟各种授权状态和错误场景。
...
**更新** 新增语音服务 API 兼容性测试、语音输入字段允许列表测试、错误处理场景测试，以及紧急确认文案测试用例，确保紧急求助功能的文案一致性与无障碍体验，提升紧急情况下的用户体验与安全性。
...
- 测试覆盖率建议
  ...
  - 错误路径与边界条件：充分覆盖
  - 紧急求助功能：≥95%（包含文案、无障碍、状态转换）
  - 语音服务功能：≥90%（包含 API 兼容性、字段允许列表、错误处理）
- 最佳实践清单
  ...
  - 使用 measure 块进行性能测试
  - 在 UI 测试中记录关键页面截图
  - 紧急功能测试应包含文案一致性与无障碍验证
  - 确保紧急求助按钮的可见性与可达性
  - 语音服务测试应覆盖 API 兼容性和错误处理场景
  - 语音输入测试应验证字段允许列表和权限处理
  - 语音服务应避免重复播报，提供清晰的用户反馈