...
## 更新摘要
**所做更改**
- 新增 currentUser 更新方法的详细说明，包括用户数据同步和活跃角色自动解析功能
- 更新用户会话管理章节，反映最新的状态更新机制
- 增强状态持久化和同步策略的说明
- 完善用户状态管理的最佳实践指导
...
**新增** 应用实现了智能的用户数据同步机制，通过 `updateCurrentUser` 方法确保用户状态的准确性和一致性。
...
participant AppState as AppState
participant View as 视图层
API->>AppState : 返回UserDto
AppState->>AppState : updateCurrentUser(user)
Note over AppState : 解析activeRole优先级
AppState->>View : @Published currentUser变更
AppState->>View : @Published activeRole变更
View-->>View : 自动重新渲染
```

**图表来源**
- [AppState.swift:100-113](file://blindRun/blindRun/Core/AppState.swift#L100-L113)

用户数据同步的关键特性：
- **智能角色解析**：当用户数据中的 `activeRole` 为 `nil` 时，自动使用提供的回退角色
- **完整用户信息更新**：同时更新用户基本信息和激活角色状态
- **原子性状态变更**：确保用户数据和角色状态的一致性
...
**新增** 基于最新的 currentUser 更新方法，以下是推荐的最佳实践：

- **使用 updateCurrentUser 进行状态同步**：当从后端获取最新用户数据时，始终使用 `updateCurrentUser` 方法确保状态一致性
- **合理设置回退角色**：在调用 `updateCurrentUser` 时提供合适的回退角色，避免激活角色丢失
- **原子性更新**：利用 `updateCurrentUser` 的原子性特性，确保用户数据和角色状态同时更新
- **状态监听**：订阅 AppState 的状态变化，及时响应用户状态的更新
...
**排查步骤**：
1. 确认是否使用 `updateCurrentUser` 方法进行状态更新
2. 检查用户数据中的 `activeRole` 字段是否正确解析
3. 验证回退角色参数是否正确传递
4. 查看 AppState 的状态变更日志
...
- [LoginViewModel.swift:161-189](file://blindRun/blindRun/Auth/LoginViewModel.swift#L161-L189)
...
7. **智能用户状态同步**：通过 `updateCurrentUser` 实现用户数据和角色状态的精确同步
