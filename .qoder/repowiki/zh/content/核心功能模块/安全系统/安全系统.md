...
## 更新摘要
**所做更改**
- 整合AGENTS.md中的安全规则和应急程序规范
- 更新紧急求助流程的触发条件和确认机制
- 完善危险操作保护策略和用户确认对话框要求
- 增强安全配置参数和权限控制说明
- 补充监控告警、日志记录和审计追踪功能
...
**更新** 整合AGENTS.md中的安全规则，明确了紧急求助的触发条件、确认机制和应急程序的具体要求。
...
- 二次确认
  ...
  - 确认对话框文本必须严格遵循 AGENTS.md 规定："是否确认进入求助状态？确认后，本次服务将标记为异常，系统会记录当前订单状态。"
- 状态流转
...
U->>S : 点击"紧急求助"
S->>U : 弹出确认对话框
Note over S,U : "是否确认进入求助状态？确认后，本次服务将标记为异常，系统会记录当前订单状态。"
U->>S : 确认求助
...
**章节来源**
- [safety-basic-emergency/spec.md:3-9](file://openspec/changes/add-aidrun-ios-spring-mvp/specs/safety-basic-emergency/spec.md#L3-L9)
- [safety-basic-emergency/spec.md:11-17](file://openspec/changes/add-aidrun-ios-spring-mvp/specs/safety-basic-emergency/spec.md#L11-L17)
...
- 紧急联系人要求：盲人必须在个人资料中提供紧急联系人姓名和电话号码，否则无法预订。
...
**章节来源**
- [04-user-flows-and-state-machine.md:259-273](file://docs/04-user-flows-and-state-machine.md#L259-L273)
- [AGENTS.md:324-343](file://AGENTS.md#L324-L343)

### 监控告警、日志与审计
- 安全事件记录
  - 紧急求助：记录进入 emergency 的事件与前一状态。
  - 取消事件：记录取消方、取消原因与时间戳。
  - 紧急联系人：记录紧急联系人信息但不进行实际通知。
- 日志与审计
...
**章节来源**
- [safety-basic-emergency/spec.md:23-25](file://openspec/changes/add-aidrun-ios-spring-mvp/specs/safety-basic-emergency/spec.md#L23-L25)
...
- 紧急联系人缺失
  - 盲人无法预订订单，系统返回 PROFILE_INCOMPLETE 错误。

**章节来源**
- [04-user-flows-and-state-machine.md:259-273](file://docs/04-user-flows-and-state-machine.md#L259-L273)
- [08-ios-architecture.md:80-82](file://docs/08-ios-architecture.md#L80-L82)
- [AGENTS.md:328-333](file://AGENTS.md#L328-L333)

## 结论
本安全系统以状态机为核心，结合二次确认与终态保护，确保紧急求助与取消等危险操作在可控范围内执行。通过明确的触发条件、严格的权限与角色限制、以及可审计的日志记录，系统在 MVP 阶段即具备良好的安全基线。AGENTS.md 的整合进一步强化了安全规则的权威性和执行标准，特别是紧急求助的确认机制和危险操作的保护策略。建议在生产环境进一步完善令牌存储、告警与审计能力，并持续优化用户体验与安全性。
