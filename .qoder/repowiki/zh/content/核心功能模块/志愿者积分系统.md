...
**所做更改**
- 更新了订单完成后的积分计算逻辑实现细节
- 新增了RunOrderService中的积分处理和志愿者积分账本管理
- 完善了积分系统的数据模型和存储结构
- 增强了积分获取规则的技术实现说明
...
**更新** 基于RunOrderService的实现，积分计算逻辑现已完全集成到订单管理系统中：

```mermaid
flowchart TD
A[订单状态: in_progress] --> B{服务完成?}
B --> |是| C[RunOrderService.completeOrder<br/>执行积分计算]
C --> D[更新志愿者档案积分<br/>volProfile.addPoints(100)]
D --> E[创建积分流水记录<br/>VolunteerPointsLedger]
E --> F[保存到数据库]
F --> G[返回完成状态]
B --> |否| H[等待服务完成]
H --> I[继续服务流程]
...
**图表来源**
- [RunOrderService.java:240-255](file://backend/src/main/java/com/aidrun/backend/order/RunOrderService.java#L240-L255)
...
**更新** 基于新的实现，积分记录现在通过VolunteerPointsLedger实体管理：
...
**章节来源**
- [VolunteerPointsLedger.java:13-48](file://backend/src/main/java/com/aidrun/backend/volunteer/VolunteerPointsLedger.java#L13-L48)
- [VolunteerPointsLedgerRepository.java:1-7](file://backend/src/main/java/com/aidrun/backend/volunteer/VolunteerPointsLedgerRepository.java#L1-L7)
...
**更新** 基于RunOrderService的完整实现，积分处理现在通过事务性操作确保数据一致性：

#### 积分计算流程

```mermaid
flowchart TD
A[调用 completeOrder] --> B{验证订单状态}
B --> |in_progress| C[更新订单状态为 completed]
C --> D[获取志愿者档案]
D --> E[volProfile.addPoints(100)]
E --> F[创建积分流水记录]
F --> G[保存志愿者档案]
G --> H[保存积分流水]
H --> I[保存订单]
I --> J[返回结果]
B --> |非in_progress| K[抛出异常]
```

**图表来源**
- [RunOrderService.java:220-255](file://backend/src/main/java/com/aidrun/backend/order/RunOrderService.java#L220-L255)

#### 关键实现细节

- **常量定义**：`POINTS_PER_COMPLETION = 100`
- **事务管理**：使用`@Transactional`注解确保原子性
- **数据一致性**：先更新档案积分，再创建流水记录
- **错误处理**：志愿者档案不存在时抛出`PROFILE_INCOMPLETE`异常

**章节来源**
- [RunOrderService.java:32-71](file://backend/src/main/java/com/aidrun/backend/order/RunOrderService.java#L32-L71)
- [RunOrderService.java:240-255](file://backend/src/main/java/com/aidrun/backend/order/RunOrderService.java#L240-L255)
...
**章节来源**
- [DemoDataSeeder.java:100-112](file://backend/src/main/java/com/aidrun/backend/seed/DemoDataSeeder.java#L100-L112)
- [VolunteerProfile.java:107-109](file://backend/src/main/java/com/aidrun/backend/profile/VolunteerProfile.java#L107-L109)