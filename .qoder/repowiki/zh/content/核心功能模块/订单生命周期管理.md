...
## 更新摘要
**所做更改**
- 新增完整的紧急事件处理逻辑，包括紧急事件实体、状态验证和终端状态处理
- 更新紧急事件数据模型，新增 EmergencyEvent 实体和相关仓储接口
- 完善紧急事件状态转换规则，明确紧急状态的不可逆特性
- 增强紧急事件的权限验证和业务规则，确保状态转换的安全性
- 更新紧急事件的前端状态显示和交互逻辑
...
**更新** 基于新增的紧急事件处理逻辑，系统现在包含了完整的紧急事件管理功能，确保在服务过程中能够及时处理异常情况并记录相关数据。
...
SafetySpec[safety-basic-emergency/spec.md]
...
OrderDisplay[OrderDisplayHelpers.swift]
end
subgraph "后端服务"
...
EmergencyRepo[EmergencyEventRepository.java]
EmergencyDto[EmergencyEventDto.java]
EmergencyRequest[EmergencyRequest.java]
...
Backend --> EmergencyRepo
Backend --> EmergencyDto
Backend --> EmergencyRequest
...
| emergency | 无 | 无 | 终止状态，不可恢复 |

**禁止的流转**
- emergency → 任何其他状态（紧急状态为终止状态，不可恢复）
- completed → 任何状态（终态）
- cancelled → 任何状态（终态）

**更新** 新增了紧急状态的处理逻辑，紧急状态作为终止状态，不可再进行任何正常的生命周期转换。

**章节来源**
- [04-user-flows-and-state-machine.md:98-119](file://docs/04-user-flows-and-state-machine.md#L98-L119)
- [RunOrderService.java:37-39](file://backend/src/main/java/com/aidrun/backend/order/RunOrderService.java#L37-L39)
...
RunOrderService --> EmergencyRepo
RunOrderService --> EmergencyDto
RunOrderService --> EmergencyRequest
...
RUN_ORDER ||--|| EMERGENCY_EVENT : "has emergency event"
```
...
系统新增了完整的紧急事件数据模型，用于记录紧急状态的相关信息：
...
**图表来源**
- [EmergencyEvent.java:14-57](file://backend/src/main/java/com/aidrun/backend/order/EmergencyEvent.java#L14-L57)
...
API-->>App : { status : "emergency", emergencyAt : timestamp }
...
**更新** 紧急求助流程现在包含完整的权限验证和状态检查，确保只有在正确状态下才能触发紧急事件，并且紧急状态不可恢复。

**图表来源**
- [04-user-flows-and-state-machine.md:230-257](file://docs/04-user-flows-and-state-machine.md#L230-L257)
- [safety-basic-emergency/spec.md:19-37](file://openspec/changes/add-aidrun-ios-spring-mvp/specs/safety-basic-emergency/spec.md#L19-L37)

### 数据模型设计

系统定义了完整的订单数据模型，包括紧急事件的关联关系：
...
BlindOrderStatusView[BlindOrderStatusView.swift]
end
subgraph "业务逻辑层"
OrderService[RunOrderService.java]
UserService[用户服务]
...
EmergencyRepo --> EmergencyEvent
EmergencyDto --> EmergencyEvent
EmergencyRequest --> EmergencyEvent
```

**更新** 新增了紧急事件相关的依赖关系，包括 EmergencyEvent、EmergencyEventRepository、EmergencyEventDto 和 EmergencyRequest 的完整依赖链。
...
1. **快速状态转换**：紧急事件触发后立即更新状态，确保实时性
2. **最小化数据写入**：紧急事件只记录必要的状态信息
3. **权限快速验证**：在触发紧急事件前进行快速权限检查
...
**更新** 新增了紧急事件处理的性能优化策略，确保紧急状态能够快速响应和处理。
...
| 紧急状态不可恢复 | 触发紧急后无法继续 | 紧急状态为终止状态 | 显示异常处理说明 |
| 状态枚举不匹配 | 前后端状态值不一致 | RunOrderStatus映射错误 | 检查枚举值映射 |

**更新** 新增了紧急状态相关的故障排除指南，包括紧急状态不可恢复的处理建议。
...
CheckType --> |紧急状态错误| HandleEmergency["处理紧急状态"]
Retry --> Success{"重试成功?"}
...
HandleEmergency --> ShowEmergencyInfo["显示紧急状态说明"]
ShowEmergencyInfo --> Continue
...
7. **完整的紧急事件处理**：新增的紧急事件实体和状态验证机制

**更新** 新增的紧急事件处理逻辑进一步增强了系统的安全性和可靠性，通过完整的紧急事件数据模型和严格的权限验证，确保在服务过程中能够及时处理异常情况并提供完整的数据记录。

该系统为类似的服务匹配应用提供了优秀的参考实现，特别是在处理实时状态同步、并发控制和紧急事件处理方面具有重要的借鉴价值。