...
- 手机号登录与账户自动创建流程
- **自定义Base64编码JWT令牌系统**，包含发行者验证和用户ID提取功能
- 固定验证码验证逻辑
...
- [SecurityConfig.java:1-33](file://backend/src/main/java/com/aidrun/backend/config/SecurityConfig.java#L1-L33)
- [AppUser.java:1-52](file://backend/src/main/java/com/aidrun/backend/user/AppUser.java#L1-L52)
...
- [07-api-contract.openapi.yaml:25-46](file://docs/07-api-contract.openapi.yaml#L25-L46)
- [JwtService.java:19-35](file://backend/src/main/java/com/aidrun/backend/auth/JwtService.java#L19-L35)
...
**更新** 新增自定义Base64编码JWT令牌系统，包含发行者验证和用户ID提取功能

- **令牌格式**
  - 载荷结构：`issuer:userId:timestamp`
  - 编码方式：URL安全的Base64编码，无填充字符
  - 解码验证：使用URL解码器解码，UTF-8字符集
- **发行者验证**
  - 解析时将载荷分割为三部分：issuer、userId、timestamp
  - 验证发行者标识与配置的一致性
...
- **时间戳集成**
  - 令牌包含生成时间戳，便于令牌生命周期管理
  - 可用于令牌过期检查和审计

```mermaid
flowchart TD
Issue["签发令牌"] --> Payload["构建载荷<br/>issuer:userId:timestamp"]
Payload --> Encode["Base64 URL编码<br/>无填充字符"]
Encode --> Token["返回令牌"]
Parse["解析令牌"] --> Decode["Base64 URL解码"]
Decode --> Split["分割载荷<br/>parts[0]:issuer<br/>parts[1]:userId<br/>parts[2]:timestamp"]
Split --> Validate{"验证发行者一致性和<br/>载荷格式"}
Validate --> |通过| Extract["提取用户ID"]
Validate --> |失败| Empty["返回空Optional"]
...
- [JwtService.java:24-35](file://backend/src/main/java/com/aidrun/backend/auth/JwtService.java#L24-L35)
...
IOS --> Store["本地存储"]
Store --> IOS
API --> JwtService["JWT服务"]
JwtService --> SecurityConfig["安全配置"]
JwtService --> AppUser["用户实体"]
```
...
- **JWT优化**：Base64编码相比标准Base64更高效，URL安全编码避免额外字符转换开销
...
- **令牌解析失败**
  ...
  - **可能原因**：
    - 发行者不匹配
    - Base64格式错误
    - 载荷格式不符合"issuer:userId:timestamp"
  - **处理**：重新登录获取新令牌
- 自动登录失败
  - 现象：启动后未进入首页
...
## 结论
本认证系统以"手机号 + 固定验证码"为核心入口，结合**自定义Base64编码JWT令牌系统**实现轻量级会话管理。新增的JWT服务提供发行者验证和用户ID提取功能，增强了令牌系统的安全性与可靠性。用户故事与 OpenAPI 合约明确了登录、自动登录、退出登录与角色切换的行为边界。前端通过调用后端 API 完成认证流程，后端通过JWT服务统一解析令牌，确保受保护接口的安全性。建议在后续版本中完善令牌刷新、HTTPS 与本地存储安全等措施，持续提升安全性与用户体验。
...
- 示例路径（用于定位实现）
  ...
  - **JWT服务实现**：[JwtService.java:19-35](file://backend/src/main/java/com/aidrun/backend/auth/JwtService.java#L19-L35)
  - **安全配置**：[SecurityConfig.java:13-31](file://backend/src/main/java/com/aidrun/backend/config/SecurityConfig.java#L13-L31)
  - **应用配置**：[application.yml:30-34](file://backend/src/main/resources/application.yml#L30-L34)
  - **用户实体**：[AppUser.java:16-52](file://backend/src/main/java/com/aidrun/backend/user/AppUser.java#L16-L52)