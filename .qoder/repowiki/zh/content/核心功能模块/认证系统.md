...
- [AuthService.java:1-49](file://backend/src/main/java/com/aidrun/backend/auth/AuthService.java#L1-L49)
...
- [AppUser.java:1-57](file://backend/src/main/java/com/aidrun/backend/user/AppUser.java#L1-L57)
- [UserDto.java:1-28](file://backend/src/main/java/com/aidrun/backend/user/dto/UserDto.java#L1-L28)
...
Combine --> Token["返回令牌"]
Parse["解析令牌"] --> Split["分割为三部分"]
Split --> VerifySignature["验证签名"]
VerifySignature --> VerifyIssuer{"验证发行者一致性和<br/>用户ID格式"}
VerifyIssuer --> |通过| Extract["提取用户ID"]
VerifyIssuer --> |失败| Empty["返回空Optional"]
Extract --> Result["返回用户ID"]
Empty --> Result
```
...
- [BaseEntity.java:12-54](file://backend/src/main/java/com/aidrun/backend/common/BaseEntity.java#L12-L54)
...
**章节来源**
- [03-user-stories.md:83-96](file://docs/03-user-stories.md#L83-L96)
...
- [SecurityConfig.java:1-55](file://backend/src/main/java/com/aidrun/backend/config/SecurityConfig.java#L1-L55)
- [AppUser.java:1-57](file://backend/src/main/java/com/aidrun/backend/user/AppUser.java#L1-L57)
- [AppUserRepository.java:1-10](file://backend/src/main/java/com/aidrun/backend/user/AppUserRepository.java#L1-L10)
- [UserDto.java:1-28](file://backend/src/main/java/com/aidrun/backend/user/dto/UserDto.java#L1-L28)
- [UserRole.java:1-31](file://backend/src/main/java/com/aidrun/backend/user/UserRole.java#L1-L31)
...
## 结论
本认证系统以"手机号 + 固定验证码"为核心入口，结合**基于HMAC-SHA256的JWT令牌系统**实现轻量级会话管理。新增的JWT服务提供发行者验证和用户ID提取功能，增强了令牌系统的安全性与可靠性。用户故事与 OpenAPI 合约明确了登录、自动登录、退出登录与角色切换的行为边界。前端通过调用后端 API 完成认证流程，后端通过JWT服务统一解析令牌，确保受保护接口的安全性。建议在后续版本中完善令牌刷新、HTTPS 与本地存储安全等措施，持续提升安全性与用户体验。
...
- 示例路径（用于定位实现）
  ...
  - **认证控制器实现**：[AuthController.java:22-26](file://backend/src/main/java/com/aidrun/backend/auth/AuthController.java#L22-L26)
  ...
  - **基础实体**：[BaseEntity.java:12-54](file://backend/src/main/java/com/aidrun/backend/common/BaseEntity.java#L12-L54)
  - **集成测试**：[AuthControllerIntegrationTest.java:22-102](file://backend/src/test/java/com/aidrun/backend/auth/AuthControllerIntegrationTest.java#L22-L102)