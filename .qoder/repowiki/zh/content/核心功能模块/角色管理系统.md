...
**更新** 完成了双角色切换机制的完整实现，包括后端API契约定义、业务规则验证和前端拦截逻辑。
...
**更新** 完成了完整的角色切换API实现，包括SwitchRoleRequest DTO、UserController端点和UserService业务逻辑。
...
**更新** 完成了后端UserService的实现，包括活跃订单状态检查和角色切换验证逻辑。
...
**更新** 完善了角色切换的业务规则，包括角色权限验证和活跃订单状态检查。
...
**更新** 完成了双角色切换机制的完整实现，包括后端API契约定义、业务规则验证和前端拦截逻辑。系统现已具备完整的活跃订单拦截能力，确保服务过程中的角色切换安全性和完整性。
