# 角色管理系统

<cite>
**本文档引用的文件**
- [04-user-flows-and-state-machine.md](file://docs/04-user-flows-and-state-machine.md)
- [07-api-contract.openapi.yaml](file://docs/07-api-contract.openapi.yaml)
- [08-ios-architecture.md](file://docs/08-ios-architecture.md)
- [UserController.java](file://backend/src/main/java/com/aidrun/backend/user/UserController.java)
- [UserService.java](file://backend/src/main/java/com/aidrun/backend/user/UserService.java)
- [UserRole.java](file://backend/src/main/java/com/aidrun/backend/user/UserRole.java)
- [UserControllerIntegrationTest.java](file://backend/src/test/java/com/aidrun/backend/user/UserControllerIntegrationTest.java)
- [SwitchRoleRequest.java](file://backend/src/main/java/com/aidrun/backend/user/dto/SwitchRoleRequest.java)
- [UserDto.java](file://backend/src/main/java/com/aidrun/backend/user/dto/UserDto.java)
- [AppUser.java](file://backend/src/main/java/com/aidrun/backend/user/AppUser.java)
- [RunOrderStatus.java](file://backend/src/main/java/com/aidrun/backend/order/RunOrderStatus.java)
- [ErrorCode.java](file://backend/src/main/java/com/aidrun/backend/common/error/ErrorCode.java)
- [RoleModule.swift](file://blindRun/blindRun/Role/RoleModule.swift)
- [RoleSelectionView.swift](file://blindRun/blindRun/Role/RoleSelectionView.swift)
- [RoleSelectionViewModel.swift](file://blindRun/blindRun/Role/RoleSelectionViewModel.swift)
- [blindRunApp.swift](file://blindRun/blindRun/blindRunApp.swift)
- [ContentView.swift](file://blindRun/blindRun/ContentView.swift)
- [UserModels.swift](file://blindRun/blindRun/Core/Models/UserModels.swift)
</cite>

## 更新摘要
**所做更改**
- 完成了角色切换拦截机制的完整实现，包括后端API契约和业务逻辑
- 更新了活跃订单拦截的技术细节和测试覆盖
- 补充了角色切换的业务规则和约束条件
- 完善了角色状态持久化的技术实现
- 更新了跨功能模块的角色共享机制说明

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构概览](#架构概览)
5. [详细组件分析](#详细组件分析)
6. [依赖关系分析](#依赖关系分析)
7. [性能考虑](#性能考虑)
8. [故障排除指南](#故障排除指南)
9. [结论](#结论)

## 简介

角色管理系统是"助盲跑"应用的核心功能模块，负责管理盲人跑者和志愿者两种用户角色之间的切换机制、权限控制和状态管理。该系统确保用户能够在不同角色间无缝切换，同时维护正在进行的服务完整性。

系统基于MVVM架构模式，采用SwiftUI作为UI框架，实现了完整的角色切换拦截机制、活跃订单检测和权限边界控制。所有功能都遵循无障碍设计原则，特别考虑了盲人用户的使用需求。

**更新** 完成了双角色切换机制的完整实现，包括后端API契约定义、业务规则验证、前端拦截逻辑和完整的测试覆盖。

## 项目结构

根据架构文档，项目采用模块化组织方式，角色管理功能主要分布在以下模块中：

```mermaid
graph TB
subgraph "应用架构"
App[blindRunApp]
Content[ContentView]
end
subgraph "核心模块"
Core[Core模块<br/>应用环境、依赖容器、共享模型]
Role[Role模块<br/>角色切换和角色守卫规则]
Auth[Auth模块<br/>手机登录、JWT持久化、会话管理]
end
subgraph "业务模块"
BR[BlindRunner模块<br/>盲人跑者主页、资料、预约、订单状态]
VOL[Volunteer模块<br/>志愿者主页、可用性、订单列表、服务记录]
Orders[Orders模块<br/>订单DTO、订单状态机助手、轮询机制]
end
subgraph "基础设施"
Map[Map模块<br/>高德地图桥接、当前位置、标记、距离计算]
Voice[Voice模块<br/>TTS、重复当前状态、语音输入助手]
Safety[Safety模块<br/>紧急确认和取消确认流程]
end
App --> Content
Content --> Core
Core --> Role
Core --> Auth
Role --> BR
Role --> VOL
BR --> Orders
VOL --> Orders
VOL --> Map
BR --> Voice
VOL --> Voice
Orders --> Safety
```

**图表来源**
- [08-ios-architecture.md:18-32](file://docs/08-ios-architecture.md#L18-L32)

**章节来源**
- [08-ios-architecture.md:18-32](file://docs/08-ios-architecture.md#L18-L32)

## 核心组件

### 角色切换机制

角色切换是系统的核心功能，通过统一的JWT令牌实现跨角色访问。系统支持以下角色：

- **盲人跑者 (blind_runner)**: 发起服务请求、预约跑步、管理个人资料
- **志愿者 (volunteer)**: 接受订单、提供服务、管理可用性状态

切换机制的关键特性：
- 单一JWT令牌跨角色共享
- 仅修改activeRole字段
- 活跃订单阻塞切换逻辑

**更新** 完成了完整的角色切换API实现，包括SwitchRoleRequest DTO、UserController端点和UserService业务逻辑，支持4种活跃订单状态的精确拦截。

### 权限控制边界

系统实现了严格的权限边界控制：

```mermaid
flowchart TD
Start[用户尝试切换角色] --> CheckActive[检查活跃订单]
CheckActive --> HasActive{是否存在活跃订单?}
HasActive --> |是| Block[阻止切换]
HasActive --> |否| Allow[允许切换]
Block --> Alert[显示警告弹窗<br/>"当前存在进行中的订单，暂时不能切换身份"]
Allow --> Navigate[导航到目标角色首页]
Alert --> Stay[保持当前角色]
Navigate --> Complete[切换完成]
```

**图表来源**
- [04-user-flows-and-state-machine.md:259-273](file://docs/04-user-flows-and-state-machine.md#L259-L273)

### 订单状态管理

系统实现了完整的订单生命周期管理，支持7种状态：

```mermaid
stateDiagram-v2
[*] --> matching : 盲人提交预约
matching --> accepted : 志愿者接单成功
matching --> cancelled : 盲人取消/超时自动取消
accepted --> arrived : 志愿者点击"我已到达"
accepted --> cancelled : 任一方取消
arrived --> in_progress : 盲人确认开始服务
arrived --> cancelled : 任一方取消
in_progress --> completed : 志愿者结束服务
in_progress --> emergency : 任一方触发求助
completed --> [*] : 订单结束
cancelled --> [*] : 订单结束
emergency --> [*] : 异常终态
```

**图表来源**
- [04-user-flows-and-state-machine.md:72-96](file://docs/04-user-flows-and-state-machine.md#L72-L96)

**章节来源**
- [07-api-contract.openapi.yaml:61-81](file://docs/07-api-contract.openapi.yaml#L61-L81)
- [08-ios-architecture.md:92-97](file://docs/08-ios-architecture.md#L92-L97)

## 架构概览

系统采用MVVM架构模式，确保关注点分离和代码可维护性：

```mermaid
graph TB
subgraph "视图层 (View)"
BRView[盲人跑者视图]
VOLView[志愿者视图]
SettingsView[设置视图]
RoleSelectionView[角色选择视图]
end
subgraph "视图模型层 (ViewModel)"
BRVM[BlindRunnerViewModel]
VOLVM[VolunteerViewModel]
RoleVM[RoleSelectionViewModel]
AuthVM[AuthViewModel]
end
subgraph "服务层 (Service)"
APIService[APIService]
AuthService[AuthService]
RoleService[RoleService]
end
subgraph "数据层 (Data)"
APIClient[APIClient]
UserDefaults[UserDefaults存储]
Cache[缓存机制]
end
BRView --> BRVM
VOLView --> VOLVM
SettingsView --> RoleVM
RoleSelectionView --> RoleVM
RoleVM --> RoleService
BRVM --> APIService
VOLVM --> APIService
AuthVM --> AuthService
APIService --> APIClient
RoleService --> APIClient
APIClient --> UserDefaults
APIService --> Cache
```

**图表来源**
- [08-ios-architecture.md:33-49](file://docs/08-ios-architecture.md#L33-L49)

### 数据持久化策略

系统采用多层数据持久化策略：

| 层级 | 存储介质 | 数据类型 | 特点 |
|------|----------|----------|------|
| 应用层 | UserDefaults | JWT令牌、环境配置 | MVP阶段，生产需迁移到Keychain |
| 缓存层 | 内存缓存 | 订单状态、用户会话 | 快速访问，应用重启丢失 |
| 网络层 | 服务器端 | 用户资料、订单数据 | 永久存储，实时同步 |

**章节来源**
- [08-ios-architecture.md:78-83](file://docs/08-ios-architecture.md#L78-L83)

## 详细组件分析

### 角色切换拦截器

角色切换拦截器是系统安全性的关键组件，确保不会在服务进行中进行角色切换：

```mermaid
sequenceDiagram
participant User as 用户
participant App as 应用
participant API as 后端API
participant DB as 数据库
User->>App : 点击"切换角色"
App->>App : 检查当前活跃订单
App->>API : GET /api/users/me/active-role
API->>DB : 查询用户活跃订单
DB-->>API : 订单列表
API-->>App : 活跃订单状态
alt 存在活跃订单
App->>User : 显示警告弹窗
App->>User : TTS播报"当前存在进行中的订单，暂时不能切换身份"
User->>App : 确认
App->>App : 保持当前角色
else 无活跃订单
App->>API : PATCH /api/users/me/active-role
API->>DB : 更新用户activeRole
DB-->>API : 更新成功
API-->>App : 新角色用户信息
App->>User : 导航到目标角色首页
end
```

**图表来源**
- [04-user-flows-and-state-machine.md:259-273](file://docs/04-user-flows-and-state-machine.md#L259-L273)
- [07-api-contract.openapi.yaml:61-81](file://docs/07-api-contract.openapi.yaml#L61-L81)

**更新** 完成了后端UserService的实现，包括活跃订单状态检查和角色切换验证逻辑，支持4种活跃订单状态的精确拦截。

### 订单状态轮询机制

系统实现了智能轮询机制，确保订单状态的实时更新：

```mermaid
sequenceDiagram
participant App as 应用
participant API as 后端API
participant Timer as 定时器
participant TTS as 语音播报
Timer->>App : 每5秒触发
App->>API : GET /api/orders/{orderId}
API-->>App : 订单状态
alt 状态发生变化
App->>App : 更新UI状态
App->>TTS : 播报新状态
TTS-->>App : 播报完成
else 状态未变化
App->>App : 保持当前UI
end
Note over App : 订单进入终态或页面离开时停止轮询
```

**图表来源**
- [04-user-flows-and-state-machine.md:275-299](file://docs/04-user-flows-and-state-machine.md#L275-L299)

### 权限边界控制

系统实现了严格的权限边界，防止越权操作：

| 操作 | 允许角色 | 禁止状态 | 说明 |
|------|----------|----------|------|
| 切换角色 | 任意角色 | 仅当无活跃订单 | 防止服务中断 |
| 接单 | 志愿者 | 未认证、未开启可用性 | 确保服务质量 |
| 到达确认 | 志愿者 | 非accepted状态 | 防止状态混乱 |
| 开始服务 | 盲人 | 非arrived状态 | 确保服务顺序 |
| 结束服务 | 志愿者 | 非in_progress状态 | 防止重复计费 |
| 紧急求助 | 任意角色 | emergency终态 | 确保异常处理 |

**更新** 完善了角色切换的业务规则，包括角色权限验证和活跃订单状态检查，支持4种活跃订单状态的精确拦截。

**章节来源**
- [07-api-contract.openapi.yaml:256-387](file://docs/07-api-contract.openapi.yaml#L256-L387)

### 角色切换API实现

系统提供了完整的角色切换API实现，包括前后端的完整集成：

```mermaid
flowchart TD
Client[客户端] --> Request[SwitchRoleRequest]
Request --> Controller[UserController]
Controller --> Service[UserService]
Service --> Repository[数据库查询]
Repository --> Check{检查活跃订单}
Check --> |存在活跃订单| Block[返回409冲突]
Check --> |无活跃订单| Update[更新activeRole]
Block --> Error[ACTIVE_ORDER_ROLE_SWITCH_BLOCKED]
Update --> Success[返回200成功]
Success --> Client[更新后的UserDto]
```

**图表来源**
- [UserController.java:32-38](file://backend/src/main/java/com/aidrun/backend/user/UserController.java#L32-L38)
- [UserService.java:61-83](file://backend/src/main/java/com/aidrun/backend/user/UserService.java#L61-L83)

**章节来源**
- [UserController.java:15-44](file://backend/src/main/java/com/aidrun/backend/user/UserController.java#L15-L44)
- [UserService.java:21-84](file://backend/src/main/java/com/aidrun/backend/user/UserService.java#L21-L84)

## 依赖关系分析

系统采用松耦合设计，各模块间通过清晰的接口通信：

```mermaid
graph TB
subgraph "外部依赖"
URLSession[URLSession网络层]
AVSpeech[AVSpeechSynthesizer语音]
CoreLocation[CoreLocation定位]
AMap[高德地图SDK]
end
subgraph "内部模块依赖"
Role[Role模块]
Auth[Auth模块]
BR[BlindRunner模块]
VOL[Volunteer模块]
Orders[Orders模块]
Map[Map模块]
Voice[Voice模块]
end
Role --> Auth
BR --> Orders
VOL --> Orders
VOL --> Map
BR --> Voice
VOL --> Voice
Orders --> URLSession
Map --> AMap
Voice --> AVSpeech
Map --> CoreLocation
```

**图表来源**
- [08-ios-architecture.md:68-83](file://docs/08-ios-architecture.md#L68-L83)

### 错误处理策略

系统实现了多层次的错误处理机制：

```mermaid
flowchart TD
Request[API请求] --> Send[发送请求]
Send --> Response{收到响应?}
Response --> |是| Status{HTTP状态码}
Response --> |否| NetworkError[网络错误]
Status --> |2xx| Success[成功处理]
Status --> |401| AuthError[认证错误]
Status --> |409| ConflictError[冲突错误]
Status --> |其他| OtherError[其他错误]
AuthError --> Reauth[重新认证]
ConflictError --> Retry[重试机制]
NetworkError --> Retry
OtherError --> UserError[用户友好提示]
Reauth --> Send
Retry --> Send
UserError --> Display[显示错误信息]
```

**图表来源**
- [08-ios-architecture.md:68-77](file://docs/08-ios-architecture.md#L68-L77)

**章节来源**
- [08-ios-architecture.md:68-77](file://docs/08-ios-architecture.md#L68-L77)

## 性能考虑

### 轮询优化策略

系统采用了智能轮询策略以平衡实时性和性能：

- **轮询频率**: 5秒间隔，减少网络开销
- **触发条件**: 仅在订单相关页面启用
- **停止条件**: 订单进入终态、页面离开、用户登出
- **并发控制**: 避免重复请求，使用防抖机制

### 内存管理

- ViewModel生命周期与页面绑定，页面销毁时自动清理
- 使用弱引用避免循环引用
- 大对象及时释放，避免内存泄漏

### 网络优化

- 请求合并：多个小请求合并为批量请求
- 缓存策略：热点数据本地缓存
- 错败重试：指数退避算法减少服务器压力

## 故障排除指南

### 常见问题及解决方案

| 问题类型 | 症状 | 解决方案 |
|----------|------|----------|
| 角色切换失败 | 显示"当前存在进行中的订单" | 完成或取消当前订单后再切换 |
| 订单状态不更新 | 轮询无响应 | 检查网络连接，重启应用 |
| 权限不足 | 操作被拒绝 | 确认角色权限，检查认证状态 |
| 语音播报异常 | 无声音输出 | 检查系统语音设置，重启TTS服务 |

### 测试覆盖范围

系统具有完整的测试覆盖，包括：

- **单元测试**: UserService业务逻辑验证
- **集成测试**: 完整的API端到端测试
- **边界测试**: 活跃订单状态拦截验证
- **错误场景**: 网络错误、认证失败等异常处理

**章节来源**
- [UserControllerIntegrationTest.java:107-204](file://backend/src/test/java/com/aidrun/backend/user/UserControllerIntegrationTest.java#L107-L204)

## 结论

角色管理系统通过精心设计的架构和严格的权限控制，为"助盲跑"应用提供了稳定可靠的角色切换功能。系统不仅满足了基本的业务需求，还充分考虑了无障碍设计和用户体验。

**更新** 完成了双角色切换机制的完整实现，包括后端API契约定义、业务规则验证、前端拦截逻辑和完整的测试覆盖。系统现已具备完整的活跃订单拦截能力，确保服务过程中的角色切换安全性和完整性。

关键优势包括：
- **安全性**: 完整的活跃订单检测和拦截机制
- **可靠性**: 智能轮询和错误处理策略
- **可扩展性**: 模块化架构支持功能扩展
- **可维护性**: 清晰的关注点分离和依赖关系
- **可测试性**: 完整的测试覆盖确保代码质量

未来可以考虑的改进方向：
- 将JWT存储从UserDefaults迁移到Keychain
- 实现WebSocket实现实时推送
- 增加更细粒度的权限控制
- 优化轮询策略以进一步降低网络开销
- 扩展角色切换的业务场景支持