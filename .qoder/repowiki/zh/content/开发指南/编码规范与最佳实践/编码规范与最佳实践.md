...
**所做更改**
- 更新了项目源代码真相优先级规则
- 新增了严格的MVP范围规则和禁止功能清单
- 完善了iOS开发工作流程和模块化开发规范
- 强化了无障碍开发和语音交互的具体实现要求
- 更新了模块组织结构和开发工作流程
- 新增了OpenSpec优先级和验证流程
...
**更新** 基于 AGENTS.md 中的最新规则，明确了项目源代码真相优先级、严格的MVP范围约束和标准化的开发工作流程。

## 项目结构

根据 AGENTS.md 和现有架构文档，blindRun 项目采用模块化的源代码组织方式，严格遵循 SwiftUI + MVVM 架构模式：
...
**更新** 根据 AGENTS.md 规则，View 仅处理渲染和交互，不堆积业务逻辑到 View 中。

**章节来源**
- [08-ios-architecture.md:33-48](file://docs/08-ios-architecture.md#L33-L48)
- [AGENTS.md:235](file://AGENTS.md#L235)
...
- [AGENTS.md:227-238](file://AGENTS.md#L227-L238)
...
**更新** 基于 AGENTS.md 的最新规则，项目现在具有更严格的开发约束和标准化的工作流程，确保在3天MVP目标内高质量交付。
...
**新增** 根据 AGENTS.md 第1章，当多个来源冲突时，遵循以下优先级顺序：

1. **AGENTS.md** - 项目级AI编码代理最高优先级工作合同
2. **产品需求文档** - 产品需求文档
3. **MVP范围文档** - 最小可行产品范围
4. **用户故事文档** - 用户故事
5. **用户流程和状态机文档** - 用户流程和状态机
6. **页面规格文档** - 页面规格
7. **数据模型文档** - 数据模型
8. **API契约文档** - API契约
9. **iOS架构文档** - iOS架构
10. **无障碍和语音指南** - 无障碍和语音指南
11. **AI编码任务文档** - AI编码任务
12. **OpenSpec变更** - OpenSpec变更
...
**新增** 根据 AGENTS.md 第13章，所有AI代理必须遵循以下工作流程：

1. **阅读AGENTS.md** - 首先阅读项目规则
2. **阅读相关文档** - 然后阅读相关文档和OpenSpec
3. **模块化开发** - 每次只实现一个模块
4. **避免重构整个项目** - 不要一次性重写整个项目
5. **小步快跑** - 每个模块使用一个小的PR
6. **先确认规格** - 在实现前确认相应规格
7. **更新文档** - 实现后更新必要文档
8. **运行测试** - 实现后运行测试
9. **输出结果** - 输出变更文件列表、测试结果和未完成事项
10. **控制范围** - 不要静默扩大范围
...
**新增** 根据 AGENTS.md 第64-92条硬性范围规则，不得添加以下非MVP功能：

- **Android支持** - 不要构建Android版本
- **Flutter作为当前实现栈** - 不要使用Flutter作为当前实现栈
- **完整管理员后台** - 不要构建完整管理员应用
- **真实短信服务** - 不要集成真实SMS服务
- **真实身份验证** - 不要构建真实身份验证
- **WebSocket** - 不要使用WebSocket
- **实时轨迹分享** - 不要实现实时轨迹分享
- **自动电话** - 不要构建自动电话功能
- **自动短信** - 不要构建自动短信功能
- **AI助手** - 不要构建AI助手
- **复杂自然语言时间解析** - 不要构建复杂时间解析
- **路线导航** - 不要构建路线导航
- **完整积分商店** - 不要构建完整积分商店
- **支付功能** - 不要构建支付功能
- **库存管理** - 不要构建库存管理
...
**章节来源**
- [AGENTS.md:378-414](file://AGENTS.md#L378-L414)

### 无障碍开发具体实现

**新增** 基于 OpenSpec规格文件，无障碍开发的具体实现要求：
...
**章节来源**
- [spec.md:1-42](file://openspec/changes/add-aidrun-ios-spring-mvp/specs/accessibility-voice-ui/spec.md#L1-L42)
- [spec.md:1-38](file://openspec/changes/add-aidrun-ios-spring-mvp/specs/amap-location/spec.md#L1-L38)
