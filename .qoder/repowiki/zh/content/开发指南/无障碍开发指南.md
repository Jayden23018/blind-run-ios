...
## 简介
本指南面向 blindRun 项目的无障碍开发，聚焦 iOS 无障碍框架的落地实践，包括 VoiceOver、Voice Control、辅助触控、语音合成（TTS）与语音识别（ASR）的集成方法，以及无障碍标签、描述与提示的编写规范。文档基于现有设计文档与任务清单，结合 SwiftUI + MVVM 架构，给出可操作的实现建议与测试策略，确保所有 UI 元素具备适当的可访问性属性，并满足屏幕阅读器、键盘导航与触觉反馈的适配要求。
...
## 核心组件
- VoiceOver 与可访问性属性
  - 所有关键按钮、输入框、状态文本需配置 accessibilityLabel 与 accessibilityHint，并正确设置 accessibilityTraits（如按钮、头部、已选中、禁用）。
  - 关键覆盖范围包括：登录手机号与验证码输入、角色切换、盲人首页状态文本、预约表单字段、定位权限提示、提交预约按钮、取消订单、紧急求助、确认开始服务、重复当前状态、志愿者可服务开关、可接订单行、接单/到达/完成/取消/紧急等动作、评分控件。
- 语音合成（TTS）
  - 使用系统 AVSpeechSynthesizer，集中实现 SpeechService，ViewModel 在状态变化时决定播报内容，避免轮询期间重复播报。
  - 必须播报的关键节点：进入盲人首页、订单提交成功、匹配中、志愿者已接单、志愿者已到达、请确认开始服务、服务已开始、服务已完成、进入求助状态、错误提示。
- 语音识别（ASR）
  - 使用系统 Speech 框架，限定用于文本输入场景（起点描述、终点/路线描述、备注、志愿者服务摘要等），失败时显示错误并允许键盘输入。
  - 仅在用户主动发起语音输入时申请麦克风与语音识别权限。
- 大按钮与极简交互
  - 盲人端关键主按钮最小高度不低于 64 点，文字清晰，高对比度，确保触控易达。
- 危险操作二次确认
