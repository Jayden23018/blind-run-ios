...
**所做更改**
- 新增两层解码策略的详细说明和实现
- 新增云后端响应格式支持，包括 `APIEnvelopeResponse` 和 `APIErrorEnvelope`
- 增强错误处理机制，支持多种错误响应格式的统一处理
- 更新客户端Swift错误模型以支持云后端响应包络
- 完善错误处理的最佳实践和客户端集成指南
...
本文件详细说明了 AidRun 项目的API错误处理机制，重点介绍后端新增的统一错误处理基础设施和客户端新增的两层解码策略。该系统采用统一的错误响应格式，确保客户端能够一致地处理各种业务异常情况，并提供完整的错误追踪能力。新增的云后端响应格式支持使得系统能够兼容阿里云生产环境的特殊响应格式。
...
APIEnvelopeResponse[APIEnvelopeResponse<br/>云后端响应包络]
APIErrorEnvelope[APIErrorEnvelope<br/>云后端错误包络]
end
...
Client --> APIEnvelopeResponse
Client --> APIErrorEnvelope
```
...
新增的云后端响应包络格式支持阿里云生产环境的特殊响应格式：

```swift
/// 通用云后端业务端点响应包络
/// 格式: {"success": bool, "code": int, "message": string, "data": T}
...
```swift
/// 带Int错误码的错误包络（云后端错误格式）
/// 用于处理 {"success": false, "code": 400, "message": "..."} 格式
...
**章节来源**
- [ApiErrorResponse.java:5-14](file://backend/src/main/java/com/aidrun/backend/common/error/ApiErrorResponse.java#L5-L14)
- [ErrorModels.swift:66-82](file://blindRun/Core/Models/ErrorModels.swift#L66-L82)
...
客户端实现了智能的两层解码策略，确保兼容不同的响应格式：

```mermaid
flowchart TD
StatusCode{HTTP状态码} --> |2xx| EnvelopeCheck[尝试云后端响应包络]
StatusCode --> |401| Unauthorized[处理未授权]
StatusCode --> |其他| ErrorCheck[尝试错误响应解码]
EnvelopeCheck --> EnvelopeSuccess{包络解码成功?}
EnvelopeSuccess --> |是| ReturnPayload[返回data字段]
EnvelopeSuccess --> |否| DirectDecode[直接解码]
ErrorCheck --> StringCodeError{字符串错误码解码成功?}
StringCodeError --> |是| ThrowServerError[抛出服务端错误]
StringCodeError --> |否| IntCodeEnvelope[尝试云后端错误包络]
IntCodeEnvelope --> ReturnUnknown[返回未知错误]
```

**图表来源**
- [APIClient.swift:177-207](file://blindRun/Core/APIClient.swift#L177-L207)
...
本系统的错误处理机制通过新增的统一基础设施和两层解码策略，为客户端提供了更加完善和一致的错误处理体验。`ApiErrorResponse` 的引入增加了时间戳追踪能力，`ApiException` 提供了类型安全的异常封装，`ErrorCode` 的枚举化确保了错误码的一致性和可维护性，`GlobalExceptionHandler` 的增强支持了参数验证异常的统一处理。

新增的云后端响应包络支持使得系统能够兼容阿里云生产环境的特殊响应格式，包括 `{"success": false, "code": 400, "message": "..."}` 格式的错误响应处理。两层解码策略确保了系统在不同环境下的兼容性和稳定性。
...
4. 在UI层面提供友好的错误反馈
5. 实现错误恢复和重试机制
6. 充分利用两层解码策略的优势，确保对不同响应格式的兼容处理
