...
## 更新摘要
**变更内容**
- 新增分页支持：在 OpenAPI 规范中增加了分页参数支持
- 增强动态解码：改进了 JWT 令牌的动态解码机制
- 完善紧急事件处理：新增 EmergencyRequest DTO 和 EmergencyEventDto 数据结构
- 更新 API 文档：同步更新了所有相关接口的文档说明
...
订单管理模块实现了严格的订单状态机，确保服务流程的规范性和安全性。系统支持盲人跑者和志愿者两种角色，提供完整的预约、接单、服务流程管理功能。**更新** 新增了分页支持、动态解码和紧急事件处理功能，进一步增强了系统的可扩展性和用户体验。
...
EmergencyRequest --> EmergencyEventDto
```

**图表来源**
- [RunOrderController.java:23-140](file://backend/src/main/java/com/aidrun/backend/order/RunOrderController.java#L23-L140)
- [RunOrderService.java:29-403](file://backend/src/main/java/com/aidrun/backend/order/RunOrderService.java#L29-L403)
- [RunOrderRepository.java:11-37](file://backend/src/main/java/com/aidrun/backend/order/RunOrderRepository.java#L11-L37)
...
**图表来源**
- [aidrun-mvp.yaml:16-24](file://backend/src/main/resources/static/openapi/aidrun-mvp.yaml#L16-L24)
...
#### EmergencyEventDto 数据结构

**更新** 新增紧急事件数据传输对象。

| 字段名称 | 类型 | 必填 | 描述 |
|----------|------|------|------|
| `id` | string(uuid) | 是 | 紧急事件 ID |
| `orderId` | string(uuid) | 是 | 订单 ID |
| `triggeredByRole` | UserRole | 是 | 触发者角色 |
| `previousStatus` | RunOrderStatus | 是 | 前一个状态 |
| `note` | string | 否 | 事件说明 |
| `createdAt` | datetime | 是 | 创建时间 |

**章节来源**
- [RunOrderDto.java:8-69](file://backend/src/main/java/com/aidrun/backend/order/dto/RunOrderDto.java#L8-L69)
- [EmergencyRequest.java:3-6](file://backend/src/main/java/com/aidrun/backend/order/dto/EmergencyRequest.java#L3-L6)
- [EmergencyEventDto.java:8-28](file://backend/src/main/java/com/aidrun/backend/order/dto/EmergencyEventDto.java#L8-L28)
...
**图表来源**
- [RunOrderService.java:359-390](file://backend/src/main/java/com/aidrun/backend/order/RunOrderService.java#L359-L390)
...
GetOrderDetail --> EmergencyRequest
GetOrderDetail --> EmergencyEventDto
```

**图表来源**
- [RunOrderController.java:33-140](file://backend/src/main/java/com/aidrun/backend/order/RunOrderController.java#L33-L140)
...
**章节来源**
- [RunOrderRepository.java:13-36](file://backend/src/main/java/com/aidrun/backend/order/RunOrderRepository.java#L13-L36)
...
7. ****更新** 分页支持**: 在 OpenAPI 规范中增加了分页参数支持
8. ****更新** 动态解码**: 改进了 JWT 令牌的动态解码机制
9. ****更新** 紧急事件处理**: 新增了完整的紧急事件数据结构和处理流程

**更新** 新的 RunOrderService 业务逻辑进一步增强了系统的业务处理能力和数据完整性，为订单生命周期的完整管理提供了坚实的后端基础。新增的紧急事件处理功能、动态解码机制和分页支持为系统的可扩展性和用户体验提供了重要保障。
