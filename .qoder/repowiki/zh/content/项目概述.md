...
## 更新摘要
**变更内容**
- 更新权威性层级：AGENTS.md确立为项目最高优先级工作合同，具有最高优先级
- 新增12个文档的权威性层级结构，从1到12级
- 强化项目规范的统一性和一致性要求
- 明确AI编码代理的工作规则和约束
...
**权威性层级更新**：项目现确立以 AGENTS.md 为最高优先级工作合同，确保所有开发活动遵循统一的项目规范和约束。
...
**权威性层级**：项目现确立12个文档的权威性层级，AGENTS.md 为最高优先级（第1级），确保所有开发活动遵循统一的项目规范。

```mermaid
graph TB
subgraph "项目权威性层级"
Level1["AGENTS.md<br/>最高优先级工作合同"]
Level2["01-product-requirements.md<br/>产品需求"]
Level3["02-mvp-scope.md<br/>MVP 范围"]
Level4["03-user-stories.md<br/>用户故事"]
Level5["04-user-flows-and-state-machine.md<br/>用户流程与状态机"]
Level6["05-page-specs.md<br/>页面规格"]
Level7["06-data-model.md<br/>数据模型"]
Level8["07-api-contract.openapi.yaml<br/>API 合同"]
Level9["08-ios-architecture.md<br/>iOS 架构"]
Level10["09-accessibility-and-voice-guidelines.md<br/>无障碍与语音规范"]
Level11["10-ai-coding-tasks.md<br/>AI 编码任务"]
Level12["openspec/changes/add-aidrun-ios-spring-mvp/<br/>OpenSpec 变更"]
end
subgraph "应用入口"
...
- **权威性合同**：AGENTS.md 作为最高优先级工作合同，确立12个文档的权威性层级，确保所有开发活动遵循统一的项目规范。
...
### 权威性层级与工作规范
项目现确立以 AGENTS.md 为最高优先级工作合同，确保所有开发活动遵循统一的项目规范和约束。权威性层级如下：

1. **AGENTS.md** - 项目最高优先级工作合同
2. **01-product-requirements.md** - 产品需求
3. **02-mvp-scope.md** - MVP 范围
4. **03-user-stories.md** - 用户故事
5. **04-user-flows-and-state-machine.md** - 用户流程与状态机
6. **05-page-specs.md** - 页面规格
7. **06-data-model.md** - 数据模型
8. **07-api-contract.openapi.yaml** - API 合同
9. **08-ios-architecture.md** - iOS 架构
10. **09-accessibility-and-voice-guidelines.md** - 无障碍与语音规范
11. **10-ai-coding-tasks.md** - AI 编码任务
12. **openspec/changes/add-aidrun-ios-spring-mvp/** - OpenSpec 变更
...
- **权威性约束**：AGENTS.md 作为最高优先级工作合同，所有其他文档和规范均需遵循其权威性层级
...
- **权威性约束**：所有性能优化决策需遵循 AGENTS.md 中的硬约束和优先级规则
...
- **权威性冲突**：当实现与 AGENTS.md 冲突时，必须按照 AGENTS.md 的最高优先级规则执行
...
**权威性更新**：随着 AGENTS.md 作为最高优先级工作合同的确立，项目现在具备了统一的权威性层级和工作规范，确保所有开发活动都遵循既定的约束和优先级规则，为项目的长期发展提供了稳定的制度基础。

## 附录
- 演示脚本与验收场景：包含完整正向流程、紧急求助流程、角色切换拦截等，可用于联调与验收
- OpenSpec 变更：定义了新能力与修改能力清单，明确了目标与非目标范围，确保项目方向不偏离
- **权威性参考**：所有实现决策均需参考 AGENTS.md 中的权威性层级和工作规则
