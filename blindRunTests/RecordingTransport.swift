...
///
/// 领域 service 层各片的「端点映射」用例共用这一个（`AuthServiceTests` / `SafetyServiceTests` …）：
/// 断言某个方法打的是不是那一条 method / path / query / requiresAuth。路径字面量本身由
/// `scripts/validate-spec-coverage.mjs` 对着后端契约撞，那个脚本看不出
/// `logout()` 打成了 `/api/auth/me`，这里管的就是那一半。
///
/// 共用一份而不是每片各抄一份：抄五份的话，改 `APIClientProtocol` 签名要改五处，
/// 而漏改的那处会安静地不再被调用。
///
