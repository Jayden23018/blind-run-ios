...
final class VoiceOrderWizardTests: XCTestCase {
    ...
    // 后端 `demo/docs/voice-golden-corpus.json` 的 iOS 侧镜像，**十一族 85 条全覆盖**。
    //
    ...
    func testMockDurationParsingMatchesTheBackendGoldenCorpus() {
        ...
        let regexCases: [(transcript: String, expected: Int)] = [
            ...
            ("两个小时", 120),
            ("就跑二十分钟吧", 20),
            // 配速前缀 + 时长同现：时长那条不能被前面的「快一点」带偏。
            ("快一点跑四十分钟", 40),
            // 起终点同现（后端 SPEC B 新增）。时长在句尾，前面隔着两个地点，
            // 抽取不能因为句子变长就断在半路。
    ...
    func testMockStartTimeParsingMatchesTheBackendGoldenCorpus() {
        withCorpusClock {
            ...
            let regexCases: [(transcript: String, expected: String)] = [
                ...
                // 程度副词打头，真正的钟点在后面。与「跑1:30，…」同型：守卫必须
                // **跳过这次匹配继续找**，直接返空会让整句抽不出时间。
                ("跑慢一点，明天早上八点出发", "2026-07-25T08:00:00"),
                // 「慢点」不带「一」，但第一个「点」前面取不到数字 ——
                // 只看第一个「点」就放弃的写法会让这句整个抽不出来。
            ...
            //
            // 「8月10号」「下周三」「大后天」「下个月十号」这一批是**日期表达**：日期词只有
            // 明天/后天/今天三个，碰上它们时解析器不会失败，而是只取钟点、把日期当没说，
            // 再套「已过就滚次日」—— 于是「8月10号早上8点」差 1 天、「下周三早上8点」差 5 天。
            // 篡改后的值能过提前量校验、读回念得很顺，盲人听到「8月9日8点」无从判断这不是
            // 自己说的那天。日期表达是开放的，正则穷举不完，认输交给大模型才是正解。
            for transcript in [
            ...
            // 与上面 llm 那批的区别是**后端连大模型兜底都不走**：这几句压根没提时间，
            // 「慢一点」里的「一点」是程度副词不是凌晨一点。不挡的话系统会凭空造出一个
            // 能过提前量校验的 01:00 读给用户听 —— 而同一句话里「慢一点」还被认作配速 EASY，
            // 两条正则打架、时间那条赢了。
            for transcript in ["跑慢一点", "快一点跑四十分钟", "晚一点跑", "早一点出发"] {
                XCTAssertNil(
                    MockAPIClient.mockVoiceStartTime(in: transcript),
                    "「\(transcript)」没提时间，程度副词的「一点」不是钟点"
                )
            }
        }
    }
    ...
    func testMockAddressSpanExtractionMatchesTheBackendGoldenCorpus() {
        ...
        let regexCases: [(transcript: String, expected: String)] = [
            ...
            // 起终点同现（语料 `_end_address_note`）。终点**没有正则链路** ——
            // ADDRESS_SPAN 只产出「起点」这一种角色，两个地点同现时必须挑中出发地。
            ("明天早上8:00从人民广场跑到五角场，跑四十分钟", "人民广场"),
            // 倒装同样要挑对：SPEC B 刻意不做介词位置一致性校验，中文口语里倒装合法。
            ("明天早上8:00跑到五角场，从人民广场出发，跑四十分钟", "人民广场"),
            ("明天早上8:00从人民广场出发跑到五角场，跑一个小时", "人民广场"),
            // 反过来只有一个地点时，「到五角场跑」抽出五角场是**对的** ——
            // 口语里它就是出发地（到那儿去跑），不是终点。
        ...
        // 「明天下午三点跑到中山公园，跑半小时」只有终点、没有起点：壳「到…跑」后面跟的是
        // 「，」不是壳尾词，一条都不该命中 —— 把终点当起点会把人约到跑步的目的地去集合。
        for transcript in [
    ...
    func testMockPaceExtractionMatchesTheBackendGoldenCorpus() {
        ...
        let regexCases: [(transcript: String, expected: String)] = [
            ...
            ("走跑结合就行", "WALK_RUN"),
            ("中等速度", "MODERATE"),
            // 这两条与 START_TIME/none 是同一批句子：配速要抽得出，时间必须抽不出。
            // 「慢一点」既是配速又长得像凌晨一点，两条正则的分工就钉在这里。
            ("跑慢一点", "EASY"),
            ("快一点跑四十分钟", "FAST")
        ]
        for testCase in regexCases {
